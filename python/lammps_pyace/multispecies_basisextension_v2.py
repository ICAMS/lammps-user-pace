...
def generate_species_keys2(elements, r):
    """
...
def generate_all_species_keys(elements):
    ...
    for r in range(1, nelements + 1):
        keys.extend(generate_species_keys(elements, r))
    return keys
...
def generate_blocks_specifications_dict(potential_config: Dict) -> Dict:
    ...
    print(f"*** embeddings_ext {embeddings_ext.items()}")
    ...
    print(f"*** bonds_ext {bonds_ext.items()}")
    for key, bonds_ext_val in bonds_ext.items():
        #if len(set(key)) == 1:
        #    key = (key[0],)
        if key in block_spec_dict:
            print(f"*** key {key} bonds_ext_val {bonds_ext_val}")
...
def generate_functions_ext(potential_config):
    ...
    if ALL in functions:
        ...
        for rank in range(1, len(functions[ALL]['nradmax_by_orders'])+1):
            #for key in combinations_with_replacement(elements, rank+1):
            for key in generate_species_keys(elements, rank):
...
def update_bonds_ext(bonds_ext, functions_ext):
    ...
    for key, funcs_spec in functions_ext.items():
        ...
        for bkey in species_key_to_bonds(key):
            ...
            if len(key)>2:
                funcs_spec['radparameters'] = bond['radparameters']
                funcs_spec['core-repulsion'] = bond['core-repulsion']
                funcs_spec['rcut'] = min(funcs_spec['rcut'], bond['rcut'])
                funcs_spec['dcut'] = max(funcs_spec['dcut'], bond['dcut'])
                funcs_spec['rcut_in'] = max(funcs_spec['rcut_in'], bond['rcut_in'])
                funcs_spec['dcut_in'] = max(funcs_spec['dcut_in'], bond['dcut_in'])
                # print(f"*** key {key} bkey {bkey} bond {bond} funcs_spec {funcs_spec}")
...
def create_multispecies_basisblocks_list(potential_config: Dict,
    ...
    #print(f"*** unif_mus_ns_to_lsLScomb_dict")
    #print(f"*** {unif_mus_ns_to_lsLScomb_dict.keys()}")
    #for k,v in unif_mus_ns_to_lsLScomb_dict.items():
    #    print(f"*** {k}: {v}")
    #print("")
...
def create_species_block(elements_vec: List, block_spec_dict: Dict,
    ...
    if "nradmax_by_orders" in block_spec_dict and "lmax_by_orders" in block_spec_dict:
        ...
        for rank, nmax, lmin, lmax in zip(range(1, max_rank + 1),
            ...
            print(f"*** combinations_with_replacement({elms}, {rank}) {list(combinations_with_replacement(elms, rank))}")
